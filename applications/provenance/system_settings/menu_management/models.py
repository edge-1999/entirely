...
from docs.utils.model_communal.model_queryset import get_queryset_system_menu_node_type
...
def get_queryset_system_menu_unassociated():
    """未关联的菜单"""
    return ProvenanceSystemSettingsMenuManagementMenu.objects.exclude(
        id__in=ProvenanceSystemSettingsMenuManagementMenuParameter.objects.values('ID_MENU'))


# 菜单参数
class ProvenanceSystemSettingsMenuManagementMenuParameter(BasesModelCommunal):
    """菜单参数"""
    ID_MENU = fields.OneToOneField(
        'menu_management.ProvenanceSystemSettingsMenuManagementMenu', verbose_name='菜单',
        on_delete=models.CASCADE, null=False, blank=False, to_field='id', clearable=True, placeholder='菜单',
        queryset=get_queryset_system_menu_unassociated,
    )
    ...
    NODE_TYPE = fields.ForeignKey(
        'standard_encoding.ProvenanceSystemSettingsStandardCodeMainDict', verbose_name='菜单节点类型',
        placeholder='菜单节点类型', on_delete=models.CASCADE, null=True, blank=True, to_field='id', clearable=True,
        queryset=get_queryset_system_menu_node_type, )
    NAME_SPACE = fields.ForeignKey(
        'routing_management.ProvenanceSystemSettingsRoutingManagementNameSpaceName', verbose_name='路由',
        placeholder='路由', on_delete=models.CASCADE, null=True, blank=True, to_field='ID', clearable=True, )
