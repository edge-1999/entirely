from account_management.models import ProvenanceSystemSettingsAccountManagementSubscriber, \
    ProvenanceSystemSettingsAccountManagementRoleSubscriber, ProvenanceSystemSettingsAccountManagementRole, \
...
from menu_management.models import ProvenanceSystemSettingsMenuManagementMenu, \
    ProvenanceSystemSettingsMenuManagementMenuParameter, ProvenanceSystemSettingsMenuManagementMenuRoleDisplay
...
class RoleMenu(SubscriberExist):
    """
    返回展示的菜单
    """
            ...
            for menu_id in menu_son:
                ...
                if menu_role_id.exists():
                    ...
                    if self.role_id & distinct_role:
                        menu_mode = MenuNode(menu_id["id"], distinct_role)
                        if node:
    ...
    def def_creat_menu(self, p_id=None, node=None):
        """展示所有目录层级"""
        menu_tree = []
        menu_son = ProvenanceSystemSettingsMenuManagementMenu.objects.filter(
            STATUS_IS_DELETE='0', STATUS_IS_EFFECTIVE=1, ID_PARENT=p_id
        ...
        if menu_son.exists():
            for menu_id in menu_son:
                menu_mode = MenuNode(menu_id.get("id"), None)
                if node:
                    node.add_child(menu_mode)
                else:
                    menu_tree.append(menu_mode.dict)
                self.def_creat_menu(menu_id["id"], menu_mode)
        return menu_tree
